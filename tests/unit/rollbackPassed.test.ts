...
import { rollbackPassed } from '../../src/commands/rollbackPassed.js';

const tempDirs: string[] = [];

async function createTempTaskRoot(): Promise<string> {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'educoder-rollback-passed-'));
  tempDirs.push(dir);
  return path.join(
    dir,
    '课程 [ufr7sxlc]',
...
describe('rollbackPassed', () => {
  it('fetches the remote passed snapshot when local passed cache is missing', async () => {
    const taskRoot = await createTempTaskRoot();
    const fetchPassedFiles = vi.fn(async () => [{ path: 'test1/tasks.h', content: 'passed\n' }]);
    ...
    expect(fetchPassedFiles).toHaveBeenCalledWith({
      taskId: 'fc7pz3fm6yjh',
      homeworkId: '3727439',
  ...
  it('prefers the local passed snapshot when it already exists', async () => {
    const taskRoot = await createTempTaskRoot();
    const fetchPassedFiles = vi.fn(async () => [{ path: 'test1/tasks.h', content: 'remote\n' }]);

    await import('node:fs/promises').then(({ mkdir }) =>
      mkdir(path.join(taskRoot, '_educoder', 'passed', 'test1'), { recursive: true }),
    );
    await writeFile(path.join(taskRoot, '_educoder', 'passed', 'test1', 'tasks.h'), 'cached passed\n', 'utf8');
